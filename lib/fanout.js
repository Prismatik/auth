...
function checkCircularInheritance(parentPermissions, permissionsByType) {
  return r.table('entities')
  .filter(entity =>
    ...
    _.each(permissionsByType, (permIds, type) => {
      _.each(parents, parent => {
        if (_.contains(permIds, parent.id)) {
          throw new Error(CIRCULAR_INHERITANCE + ' type: ' + type + ', parent: ' +  parent.id);
        };
      });
    });
  });
...
exports.resolvePermissions = (itemId, updatedPermissions) => {
  ...
  return checkCircularInheritance(parentPermissions, permissionsByType)
  .then(() => item.update({
